...
test('Apply patch success', async () => {
   const stats = await compiler('../index.js?patch=./hello.patch!./hello.js');
...
test('Apply patch failure', async () => {
   const stats = await compiler('../index.js?patch=./hello.fail.patch!./hello.js');
   const output = stats.modules[0].source;
...
test('Patch file not found', async () => {
   const stats = await compiler('../index.js?patch=./missing.patch!./hello.js');
   const output = stats.modules[0].source;
   expect(output).toMatch(/^throw new Error\("Module build failed: Error: ENOENT/);
});

test('Invalid patch file', async () => {
   const stats = await compiler('../index.js?patch=./invalid.patch!./hello.js');
   const output = stats.modules[0].source;
